...
TEST_F(resolver_test, a) {
    co_spawn(context_, query_test<dns_record_e::A>({"tuposoft.com", 1, {"178.151.191.58"}}), detached);
    context_.run();
}

TEST_F(resolver_test, mx) {
    constexpr auto PREFERENCE = 10;
    co_spawn(context_, query_test<dns_record_e::MX>({"tuposoft.com", 1, {{PREFERENCE, "mail.tuposoft.com"}}}),
             detached);
    context_.run();
}

TEST_F(resolver_test, ptr) {
    co_spawn(context_, query_test<dns_record_e::PTR>({"1.1.1.1", 1, {"one.one.one.one"}}), detached);
    context_.run();
}

TEST_F(resolver_test, ns) {
    co_spawn(context_,
             query_test<dns_record_e::NS>(
...
TEST_F(resolver_test, cname) {
    co_spawn(context_, query_test<dns_record_e::CNAME>({"bucketname.s3.amazonaws.com", 1, {"s3-1-w.amazonaws.com"}}),
             detached);
    context_.run();
}
